...
# Parse the duration configuration from the user into a timedelta object.
...
def parse_timedelta(time_str):
    ...
    return timedelta(weeks=weeks, days=days, hours=hours, minutes=minutes, seconds=seconds, milliseconds=milliseconds)
# End parse_timedelta.
...
def write_report(stats_df, metrics, offsets_dict, write_path, filename, year):
...
def get_metrics(run_data_df):
    ...
    long_non_nan_df = long_offsets_df[long_offsets_df['offset'].notna()]
    ...
    long_gaps_count = count_long_gaps(nan_df)
    max_gap_duration, gap_start_date, gap_end_date = get_longest_gap_info(nan_df)
    max_offset_duration, longest_offsets = get_max_offset_duration_info(non_nan_runs)
    large_offsets_count = count_large_offsets(large_offsets_df)
    max_offset, min_offset = get_min_max_offsets(run_data_df)
    max_offset_start_date, max_offset_end_date = get_offset_dates(run_data_df, max_offset)
    min_offset_start_date, min_offset_end_date = get_offset_dates(run_data_df, min_offset)
...
def count_long_gaps(nan_df):
    return len(filter_gaps_by_duration(nan_df))
# End count_long_gaps.


def get_longest_gap_info(nan_df):
    max_gap_duration = nan_df['duration'].max()
    gap_start_date = nan_df[nan_df['duration'] == max_gap_duration]['start date'].tolist()
    gap_end_date = nan_df[nan_df['duration'] == max_gap_duration]['end date'].tolist()
    return max_gap_duration, gap_start_date, gap_end_date
# End get_longest_gap_info.


def get_max_offset_duration_info(non_nan_runs):
...
def get_min_max_offsets(run_data_df):
...
def get_offset_dates(run_data_df, offset_value):
    bool_mask = run_data_df['offset'] == offset_value
    start_dates = run_data_df.loc[bool_mask, 'start date'].tolist()
    end_dates = run_data_df.loc[bool_mask, 'end date'].tolist()
...
# ***************************************************************************
# ******************* FUNCTION GET_UNIQUE_OFFSETS_DICT **********************
# ***************************************************************************

# Returns a dictionary listing the durations, star/end dates of every unique offset
# in the dataframe passed in.
# Requires a dataframe (filtered or not) generated by get_run_data().
...
def get_run_data(offset_column, reference_column, ref_dates, size, create_df=True):
...
# ***************************************************************************
# ******************* FUNCTION DO_FILTER_BY_DURATION ************************
# ***************************************************************************
...
def filter_offsets_by_value(df):
    ...
    return filter_by_value(df, threshold, type, use_abs, is_strict)
# End do_filter_by_value.
...
def filter_by_value(dataframe, threshold=0.0, type='min', use_abs=True, is_strict=False):
    ...
    series = abs(filtered_df['offset']) if use_abs else filtered_df['offset']
