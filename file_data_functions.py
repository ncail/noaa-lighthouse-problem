...
def get_metrics(run_data_df, time_threshold=timedelta(days=1), val_threshold=0.05):
    ...
    large_offsets_df = filter_value(run_data_df, threshold=val_threshold)
...
def get_long_offsets_dict(run_data_df, threshold=timedelta(days=1)):

    # Filter for offsets (runs) >= 1 day.
    long_offsets_df = filter_duration(run_data_df, threshold)
...
def filter_duration(dataframe, threshold, is_max=config['dur_thr_is_max'],
                    is_min=config['dur_thr_is_min'], is_strict=config['dur_thr_strict']):
...
def filter_value(dataframe, threshold, is_max=config['val_thr_is_max'],
                 is_min=config['val_thr_is_min'], use_abs=config['use_abs'],
                 is_strict=config['val_thr_strict']):
