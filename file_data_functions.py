...
def get_metrics(run_data_df, col_config):
    ...
    # Filter dataframes.
    long_offsets_df = filter_offsets_by_duration(run_data_df, col_config)
    ...
    large_offsets_df = filter_offsets_by_value(run_data_df, col_config)

    # Calculate metrics.
    long_offsets_count = count_long_offsets(long_non_nan_df)
    long_gaps_count = count_long_gaps(run_data_df, col_config['duration_column'])
...
def count_long_gaps(run_data_df, duration_col):
    return len(filter_gaps_by_duration(run_data_df, duration_col))
# End count_long_gaps.
...
def get_long_offsets_dict(run_data_df, col_config):
    ...
    long_offsets_df = filter_offsets_by_duration(run_data_df, col_config['duration_column'])

    # Get dictionary of info about the unique offsets in long_offsets_df.
    long_offsets_dict = get_unique_offsets_dict(long_offsets_df, col_config['offset_column'])
...
def get_unique_offsets_dict(run_data_df, offset_col):

    # Get the unique offset values from long offsets.
    unique_offsets = run_data_df[offset_col].unique()
...
def filter_offsets_by_duration(df, col_config):

    validate_dataframe(df, col_config)
    ...
    return filter_by_duration(df, col_config['duration_column'], threshold, type, is_strict)
...
def filter_gaps_by_duration(df, col_config):
...
def filter_offsets_by_value(df, col_config):
    ...
    return filter_by_value(df, col_config['offset_column'], threshold, type, use_abs, is_strict)
