...
def write_report(stats_df, metrics, write_path, filename, year):
    ...
    with open(f'{write_path}/{filename}.txt', 'a') as file:
        ...
        max_key_length = max(len(key) for key, value in metrics_pairs)
        ...
        for offset, data in metrics_dict.items():
            is_first = True
            file.write(f"---------------------------------------------------------------------------\n")
...
def get_metrics(primary_col, ref_col, ref_dates, size,
    ...
    return metric_data, long_offsets_dict
# End get_metrics.
